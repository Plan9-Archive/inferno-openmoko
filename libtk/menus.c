...
static const char*
tkmkmenubutton(TkTop *t, __in_z const char *arg, char **ret, int type, TkOption *opts)
{
    Tk *tk;
    const char *e;
    char **v;
...
const char*
tkchoicebutton(TkTop *t, __in_z const char *arg, char **ret)
{
    return tkmkmenubutton(t, arg, ret, TKchoicebutton, choiceopts);
}

const char*
tkmenubutton(TkTop *t, __in_z const char *arg, char **ret)
{
    return tkmkmenubutton(t, arg, ret, TKmenubutton, mbopts);
}

static TH(tkmenubutcget)
{
    TkOptab tko[3];
    TkLabel *tkl = TKobj(TkLabel, tk);
...
static TH(tkchoicebutvaluecount)
{
    TkLabel *tkl = TKobj(TkLabel, tk);
    USED(arg);
...
const char*
tkmenu(TkTop *t, __in_z const char *arg, char **ret)
{
    Tk *tk;
    const char *e;
    TkWin *tkw;
...
static const char*
tkmenuentryconf(Tk *menu, Tk *tk, __in_z const char *arg)
{
    const char *e;
    TkOptab tko[4];
...
static const char*
menuadd(Tk *menu, __in_z const char *arg, int where)
{
    Tk *tkc;
    int configure;
    const char *e;
...
static Tk*
tkmenuindex2ptr(Tk *tk, const char **arg)
{
    TkWin *tkw;
    int index;
    char *buf;
...
static int
iteminvoke(Tk *tk, Tk *tki, __in_z_opt const char *arg)
{
    int unmap = 0;
    menuitemdirty(tki);
    switch(tki->type) {
