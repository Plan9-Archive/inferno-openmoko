...
#define MM(v,base,size)  if(!Monitor || memmonitor==nil){} else memmonitor((v),0,(base),(size))
...
void*   v_poolalloc(Pool* pool, size_t size, const char*file, int line, const char*function)
{
    ...
    if(pool->cursize > pool->ressize && (prog = currun()) != nil && prog->flags&Prestricted)
        ptr = nil;
    else
        ptr = dopoolalloc(pool, size);
    if(nil==ptr)
        print("poolalloc(%s,0x%ux)=%p\n", pool->name, size, ptr);
    return ptr;
}
...
void*   v_smalloc(size_t size, const char*file, int line, const char*function)
{
    void *v;
    ...
    for(;;){
        ...
        if(v != nil)
            break;
        if(0)
            print("smalloc waiting from \n");
        osenter();
        osmillisleep(100);
        osleave();
    }
    setmagictag(v, 'SMAL');
    setmalloctag(v, 0 /*getcallerpc(&size)*/);
    setrealloctag(v, 0);
    return v;
}

void*   v_kmalloc(size_t size, const char*file, int line, const char*function)
{
    void *v;
...
void*   v_malloc(size_t size, const char*file, int line, const char*function)
{
    void *v;
    ...
    if(v != nil){
        ...
        print("malloc failed from \n");
    //print("malloc(%d)=%p\n", size, v);
    return v;
}


void*   v_mallocz(size_t size, int clr, const char*file, int line, const char*function)
{
    void *v;
    ...
    if(v != nil){
        ...
        print("mallocz failed from \n");
    //print("mallocz(%d,%d)=%p\n", size, clr, v);
    return v;
}


void    v_free(void *v, const char*file, int line, const char*function)
{
    Bhdr *b;
...
void*   v_realloc(void *v, size_t size, const char*file, int line, const char*function)
{
    void *nv;
    ...
    if(nv != nil) {
        ...
        setmagictag(nv, 'REAL');
        setrealloctag(nv, 0/*getcallerpc(&v)*/);
        if(v == nil)
            setmalloctag(nv, 0/*getcallerpc(&v)*/);
    } else
        print("realloc failed from \n");
    //print("realloc(%d,%d)=%p\n", v, size, nv);
    return nv;
}
...
void*   v_calloc(size_t num, size_t size, const char*file, int line, const char*function)
{
