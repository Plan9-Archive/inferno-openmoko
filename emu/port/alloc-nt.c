...
//  print("v_poolfree(%s,%p)\n", pool->name, v);
    ...
    BOOL rc=HeapFree(pool->handle, 0, b);
    if (0==rc) {
        panic("poolfree: HeapFree(%p, %s:%d %s) aborted with error %d", v, file, line, function, GetLastError());
    }
}
...
int poolwalk(Pool* pool, poolwalk_callback callback)
{
    ...
    while (HeapWalk( pool->handle, &phe ))
    {
        ...
        if (phe.wFlags & PROCESS_HEAP_ENTRY_BUSY && b->mymark==MYMARK )
        {
            ...
            __try
            {
                ...
                rc = callback(B2D(b), b->size, b->tag, b->file, b->line, b->function, b->comment);
                if(0!=rc)
                    break;
