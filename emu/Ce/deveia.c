...
eiainit(void)
{
	...
	for(i=0; (sysdev[i] != NULL) && (i<8); i++) {
		HANDLE comfh = CreateFileW(sysdev[i], 0, 0, NULL,	/* no security attrs */
...
eiaread(Chan *c, void *buf, long n, vlong offset)
{
	...
	case Ndataqid:
		...
		while(cnt==0) {
			...
			Sleep(0/*,FALSE*/);  //allow another thread access to port
			osleave();
			if(!good)
			{
				oserror();
