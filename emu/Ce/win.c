...
// WinMain does not parse command line and does not provide argc/argv nor __argc/__argv,
// only unparsed cmdline
// to use WinMain provide '-entry:WinMainCRTStartup' to link.exe
int wmain(int argc, wchar_t** wargv)
{
	...
	for (i=0; i<argc; i++)
	{
		argv[i] = narrowen(wargv[i]);
		//maxlen = wcslen(wargv[i])*3+1;
		//argv[i] = alloca(maxlen);
		//runestoutf(argv[i], wargv[i], maxlen);
	}
	argv[argc] = 0;
...
#if 0
int WINAPI
WinMain(HINSTANCE winst, HINSTANCE wprevinst, LPWSTR cmdline, int wcmdshow)
{
	inst = winst;
	previnst = wprevinst;
	cmdshow = wcmdshow;
...
attachscreen(Rectangle *r, ulong *chan, int *d, int *width, int *softscreen)
{
	...
	AdjustWindowRectEx(&bs, WS_OVERLAPPEDWINDOW, 0, 0/*???*/);
...
*/
	...
	if(AdjustWindowRectEx(&size, ws, 0, 0)) {
...
//	if(isunicode) {
	...
	ShowWindow(window, /*cmdshow*/SW_SHOW);
	UpdateWindow(window);
	// CloseWindow(window);
